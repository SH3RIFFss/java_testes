package com.mycompany.empacotadoraACME;//Importações
...
public class Equipe extends Thread {
	 ...
	 * não faz nada.
	 * 
	 * @return Se for possível retirar um pacote para cada (Empacotador), retorna
	 *         true, caso contrário, retorna false.
	 */
	private boolean empacotar(int nr_produtos) {
		Thread thd;
		int thd_criadas = pool_produtos.retirarProdutos(nr_produtos); // O número de threads a ser criado para empacotar
																		// é igual ao número de produtos retirados pela
																		// equipe.
		boolean controle;
	...
	/**
	 * Realiza o bloqueio da thread até que todas as threads (Empacotador) tenham
	 * finalizado
	 * 
	 * @return boolean
	 */
